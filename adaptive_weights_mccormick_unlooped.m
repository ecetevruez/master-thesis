...
scaling_factor = 0.76; % for scaling_factor = 0.5 Total iterations: 9
...
numRows = size(A, 1) + 2 + 8*n ; %4 * n + 4 * n + 4 * n + n + n;
numCols = size(A, 2) + 4 * n;
...
% J1 <= P1 - delta1
A_ext(size(A, 1) + 1, :) = [lin', zeros(1, 2 * n), ones(1, n), zeros(1, n)];

% J2 <= P2 - delta2
A_ext(size(A, 1) + 2, :) = [lin2', zeros(1, 2 * n), zeros(1, n), ones(1, n)];

% Underestimator 1 obj 1
startRow = size(A, 1) + 3;
A_ext(startRow:startRow + n - 1, :) = [zeros(n, n), 2 * diag(y1_L), zeros(n, n), -eye(n), zeros(n, n)];

% Underestimator 2 obj 1
startRow = startRow + n;
A_ext(startRow:startRow + n - 1, :) = [zeros(n, n), 2 * diag(y1_U), zeros(n, n), -eye(n), zeros(n, n)];

% Overestimator 1 obj 1
startRow = startRow + n;
A_ext(startRow:startRow + n - 1, :) = [zeros(n, n), diag(-(y1_U + y1_L)), zeros(n, n), eye(n), zeros(n, n)];

% Underestimator 1 obj 2
startRow = startRow + n;
A_ext(startRow:startRow + n - 1, :) = [zeros(n, n), zeros(n, n), 2 * diag(y2_L), zeros(n, n), -eye(n)];

% Underestimator 2 obj 2
startRow = startRow + n;
A_ext(startRow:startRow + n - 1, :) = [zeros(n, n), zeros(n, n), 2 * diag(y2_U), zeros(n, n), -eye(n)];

% Overestimator 1 obj 2
startRow = startRow + n;
A_ext(startRow:startRow + n - 1, :) = [zeros(n, n), zeros(n, n), diag(-(y2_U + y2_L)), zeros(n, n), eye(n)];

% L1'*x - y1 = 0
startRow = startRow + n;
A_ext(startRow:startRow + n - 1, :) = [L1', -eye(n), zeros(n, 3 * n)];

% L2'*x - y2 = 0
startRow = startRow + n;
A_ext(startRow:startRow + n - 1, :) = [L2', zeros(n, n), -eye(n), zeros(n, 2 * n)];

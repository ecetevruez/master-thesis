...
% Define scaling factor -- Preconditioner -- not the best
...
A_ext = [A, zeros(size(A, 1),2*n); ...                              % original constraints
         lin', zeros(1, n), ones(1, n); ...                         % J2 <= P2 - delta2
         zeros(n, n), 2 * diag(y1_L), -eye(n); ...                      % underestimator 1 obj 2
         zeros(n, n), 2 * diag(y1_U), -eye(n); ...                      % underestimator 2 obj 2
         zeros(n, n), diag(-(y1_U+y1_L)), eye(n); ...                   % overestimator  1 obj 2 
         L1', -eye(n), zeros(n,n) ...                                   % L2'*x - y2 = 0
         ];
...
model_f2.A = sparse(A_ext);  % Convert A matrix to sparse format
model_f2.vtype = [repmat('C', nx, 1); repmat('B', mbin, 1); repmat('C', 2*n, 1)];  % nx continuous, mbin binary variables
model_f2.sense = sense;
model.lb = [x_L; -inf*ones(2*n,1)]; 
model.ub = [x_U; inf*ones(2*n,1)];

Q2 = sparse(double(diag(gamma)));
Q2_ext = zeros(3*n, 3*n);  % Create an extended matrix filled with zeros
Q2_ext(1:n, 1:n) = Q2; 
    % First objective (cost) 
model_f2.multiobj(1).objn = [lin2; zeros(2*n,1)];  % Linear coefficients of the objective function
model_f2.multiobj(1).Q = Q2_ext; % Quadratic coefficients of the objective function
...
for i = 1:length(epsilons_f1)
    ...
    model_f2.rhs = b_ext;  % Right-hand side of the constraints
    % Solve the model
    st = cputime;

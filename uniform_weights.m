...
nw = 10;
W = cell(1, nw); % Initialize W as a 1×n cell array
step = 1 / (nw - 1);

% Assign weight pairs
for i = 0:nw-1
    W{i+1} = [i * step; 1 - i * step]; % Note the semicolon for a 2×1 double array
end
%gurobi_t = 0;
...
model.multiobj(1).Q = sparse(double(QQ)); % Quadratic coefficients of the objective function

model.multiobj(2).objn = beta;
model.multiobj(2).Q = sparse(double(diag(gamma)));
...
%Construct Gurobi model
    for obj_index = 1:2 
        model.multiobj(obj_index).weight = W{w_index}(obj_index);
    end
...
% Optimize
  ...
  gurobi_t = et+gurobi_t;
    ...
    % Display results
    disp(['Optimization for weights: ' num2str(W{w_index}')]);
    if strcmp(result.status, 'OPTIMAL')
        disp('Optimal solution found');
        disp(['Objective values: ' num2str(result.objval(:)')]);
    else
        disp(['Optimization finished with status: ' result.status]);
    end
    disp('------------------------');
end

...
nx = n - mbin;
QQ = diag([(Fscal^2)*quad(1:nx,1);quad(nx+1:end)]); 
...
A_ext = [A, zeros(size(A, 1),2*n); ...                                               % original constraints
         lin2', zeros(1, n), ones(1, n); ...                      % J2 <= P2 - delta2
         zeros(n, n), 2 * diag(y2_L), -eye(n); ...          % underestimator 1 obj 2
         zeros(n, n), 2 * diag(y2_U), -eye(n); ...          % underestimator 2 obj 2
         zeros(n, n), diag(-(y2_U+y2_L)), eye(n); ...       % overestimator  1 obj 2 
         L2', -eye(n), zeros(n,n) ...                                  % L2'*x - y2 = 0
         ];

    % Define the model for f1
model_f1.modelsense = 'min';  
model_f1.A = sparse(A_ext);  % Convert A matrix to sparse format
model_f1.vtype = [repmat('C', nx, 1); repmat('B', mbin, 1); repmat('C', 2*n, 1)];  % nx continuous, mbin binary variables
model_f1.sense = '<';
model.lb = [x_L; -inf*ones(2*n,1)]; 
model.ub = [x_U; inf*ones(2*n,1)];

Q1 = sparse(double(QQ));
Q1_ext = zeros(3*n, 3*n);  % Create an extended matrix filled with zeros
Q1_ext(1:n, 1:n) = Q1; 

    % First objective (cost) 
Q1 = sparse(double(QQ));
model_f1.multiobj(1).objn = [lin; zeros(2*n,1)];  % Linear coefficients of the objective function
model_f1.multiobj(1).Q = Q1_ext; % Quadratic coefficients of the objective function
...
for i = 1:length(epsilons_f2)
    ...
    model_f1.rhs = b_ext;  % Right-hand side of the constraints
